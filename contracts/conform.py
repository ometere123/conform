...
class Conform(gl.Contract):
    ...
    def register_agent(
        ...
        if len(endpoint) == 0 or len(endpoint) > MAX_ENDPOINT_LEN or not endpoint_is_safe(endpoint):
            raise gl.vm.UserError(f"{ERR_EXPECTED}: endpoint must be a public http(s) URL")
    ...
    def _run_probe(self, agent_name: str, specification: str, endpoint: str, probe: Probe) -> dict:
        ...
        def validate(leader_result) -> bool:
            ...
            try:
                ...
                return (
                    int(leader.get("verdict", 0)) == int(follower.get("verdict", -1))
                    and int(leader.get("http_class", -1)) == int(follower.get("http_class", -2))
                )
            except Exception:
                return False
